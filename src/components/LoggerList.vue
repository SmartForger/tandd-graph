<template>
  <div class="loggers">
    ...
    <div class="error-text" v-if="error === 1">Please select channels to display!</div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...mapMutations(["toggleChannel", "setView", "setThreshold", "setTitle"]),
    selectChannel(id) {
      if (this.error === 1) {
        this.error = 0;
      }
